(defn parse-description [string color-freq]
...
(defn parse-descriptions [descriptions]
  (reduce (fn [freq y] (parse-description y freq)) [0 0 0] descriptions))
...
(defn parse-game [string]
  (let
   [split (.split string ":")
    ...
    parsed-draws (map parse-descriptions split-draws)]
    [game-id parsed-draws]))
...
(def games (vec (map parse-game (vec (.split "Game 1: 12 blue, 15 red, 2 green; 17 red, 8 green, 5 blue; 8 red, 17 blue; 9 green, 1 blue, 4 red
...
(reduce (fn [acc game] (+ acc (if (valid-game game) (first game) 0))) 0 games)